...
The fix was not better instructions or more careful prompting. It was removing
the ambiguity entirely by putting sprint state into a schema-enforced SQLite
database and making the CLI the only write path. If the tool rejects a
transition, the transition does not happen. That is a smaller problem than
reconstructing what an agent thought it was editing.

The broader motivation is a working model for agentic engineering collaboration
where the human keeps structural authority over the work and the agent operates
within constraints that are enforced by tooling rather than by trust.
...
pattern is minimal isolated sessions with a compact handoff document as stateful
context. Each session gets one brief, works against the database, and stops at
an explicit checkpoint. The handoff doc is small enough that a fresh session can
orient without replaying the full history.
